...
const NutritionScreen = () => {
          ...
          try {
            ...
            const subscription = Pedometer.watchStepCount(result => {
              setSteps(currentSteps => {
                const newStepCount = currentSteps + result.steps;
                // Sauvegarder les pas dans Firebase à chaque mise à jour significative
                // Pour éviter trop d'appels à Firebase, on peut ajouter une condition
                if (result.steps >= 10) { // Par exemple, sauvegarder tous les 10 pas
                  saveStepsToFirebase(newStepCount);
                }
                return newStepCount;
              });
            });
  ...
  // Fonction pour sauvegarder le nombre de pas dans Firebase
  const saveStepsToFirebase = useCallback(async (currentSteps: number) => {
    try {
      ...
      // Récupérer l'objectif de pas à partir du contexte utilisateur
      const objectifPas = user?.ObjectifPas || 10000;
      
      // Obtenir la date actuelle au format YYYY-MM-DD
      const today = new Date();
      const dateString = today.toISOString().split('T')[0]; // Format YYYY-MM-DD
      
      // Référence à la collection des pas de l'utilisateur
      const userStepsRef = doc(db, 'utilisateurs', currentUser.uid, 'historiquePas', dateString);
      
      // Enregistrer les données de pas
      await setDoc(userStepsRef, {
        date: dateString,
        pas: currentSteps,
        objectifPas: objectifPas,
        pourcentage: Math.floor((currentSteps / objectifPas) * 100),
        caloriesBrulees: calculateBurnedCalories(currentSteps),
        derniereMiseAJour: new Date()
      }, { merge: true }); // merge:true permet d'actualiser sans écraser d'autres champs
      
      console.log(`Pas sauvegardés: ${currentSteps} pour la date ${dateString}`);
    } catch (error) {
      console.error("Erreur lors de la sauvegarde des pas dans Firebase:", error);
    }
  }, [user]);
  ...
  const fetchPedometerData = useCallback(async () => {
    try {
      if (isPedometerAvailable) {
        ...
        const pastStepData = await Pedometer.getStepCountAsync(start, end);
        if (pastStepData) {
          const currentSteps = pastStepData.steps;
          setSteps(currentSteps);
          
          // Sauvegarder les pas dans Firebase
          await saveStepsToFirebase(currentSteps);
        }
      }
    } catch (error) {
      console.error("Erreur lors de la récupération des pas:", error);
    }
  }, [isPedometerAvailable, saveStepsToFirebase]);
  ...
  const saveDailySteps = async (steps: number) => {
    ...
    try {
      ...
      console.log("Nombre de pas quotidien sauvegardé avec succès !");
    } catch (error) {
      console.error("Erreur lors de la sauvegarde des pas :", error);
    }
  };

  useEffect(() => {
    const updateStepsInFirestore = async () => {
      if (steps > 0) {
  ...
  // Sauvegarder les pas périodiquement (toutes les 5 minutes)
  useEffect(() => {
    // Ne démarrer l'intervalle que si le podomètre est disponible et l'utilisateur est connecté
    if (isPedometerAvailable && user) {
      const saveInterval = setInterval(() => {
        // Sauvegarder les pas actuels dans Firebase
        saveStepsToFirebase(steps);
  ...
  }, [isPedometerAvailable, user, steps, saveStepsToFirebase]);
            ...
            >
              ...
              <View style={styles.mealChevron}>
                ...
                <TouchableOpacity 
                  ...
                  onPress={() => router.push({
                    pathname: "/(calorie)/DetailRepas",
                    params: { date: currentDay.toISOString() }
                  })}
                >
              ...
              <View style={styles.stepsProgressTrack}>
                <View 
                  style={[
                    ...
                    { width: `${Math.min((steps / (user?.ObjectifPas || 10000)) * 100, 100)}%` }
                  ]} 
                />
              </View>
              
              <Text style={styles.stepsGoalText}>
                {Math.floor((steps / (user?.ObjectifPas || 10000)) * 100)}% de l'objectif ({user?.ObjectifPas || 10000} pas)
              </Text>
            </View>
