...
import { calculateBMR, getActivityMultiplier } from "../../services/calorie";
...
const ViewProfilePage: React.FC = () => {
  ...
  const [fieldToEdit, setFieldToEdit] = useState<
    "nomComplet" | "departement" | "poids" | "age" | "taille" | "sexe" | "niveauActivite" | "ObjectifPas"
  >("nomComplet");
        ...
        try {
          ...
          if (userData) {
            setUser({
              ...
              ObjectifPas: userData.ObjectifPas || 0
            });
          }
        } catch (error) {
  ...
  const handleEdit = useCallback((field: "nomComplet" | "departement" | "poids" | "age" | "taille" | "sexe" | "niveauActivite" | "ObjectifPas") => {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      // Si le niveau d'activité est modifié, recalculer les besoins caloriques
      if (fieldToEdit === "niveauActivite" && 
          user.poids && user.taille && user.age && user.sexe) {
        
        // Calculer les calories nécessaires en utilisant les fonctions importées
        const bmr = calculateBMR(
          parseFloat(user.poids.toString()), 
          parseFloat(user.taille.toString()), 
          parseInt(user.age.toString(), 10), 
          user.sexe
        );
        const caloriesNecessaires = Math.round(bmr * getActivityMultiplier(newValue));
        
        // Mettre à jour les calories dans Firestore
        await profileService.updateUserField(user.uid, "caloriesNecessaires", caloriesNecessaires.toString());
        
        // Mettre à jour l'état local
        setUser({ 
          ...user, 
          [fieldToEdit]: newValue,
          caloriesNecessaires: caloriesNecessaires.toString()
        });
      } else {
        ...
        >
          ...
          <View style={styles.card}>
            ...
            <EditableField
              ...
              value={user.ObjectifPas}
              suffix=" pas"
              onPress={() => handleEdit("ObjectifPas")}
            />
                        ...
                        fieldToEdit === "poids" ? "le poids" :
                          fieldToEdit === "taille" ? "la taille" :
                            fieldToEdit === "niveauActivite" ? "le niveau d'activité" :
                              fieldToEdit === "ObjectifPas" ? "l'objectif de pas" :
                                "l'âge"
                  }
                </Text>
                
                {fieldToEdit === "niveauActivite" ? (
                  <View style={styles.pickerContainer}>
                    {['sedentaire', 'legere', 'moderee', 'intense'].map((niveau) => (
                      <TouchableOpacity 
                        key={niveau}
                        style={[
                          styles.pickerOption,
                          newValue === niveau && styles.pickerOptionSelected
                        ]}
                        onPress={() => setNewValue(niveau)}
                      >
                        <Text 
                          style={[
                            styles.pickerOptionText,
                            newValue === niveau && styles.pickerOptionTextSelected
                          ]}
                        >
                  ...
                  <TextInput
                    ...
                    keyboardType={fieldToEdit === "poids" || fieldToEdit === "age" || fieldToEdit === "taille" || fieldToEdit === "ObjectifPas" ? "numeric" : "default"}
                  />
                )}
