...
const DetailColla = () => {
  ...
  useEffect(() => {
    const loadAliments = async () => {
      setLoading(true);
      try {
        const result = await fetchMealByDate(date, "Collation");
        setAliments(result.meals);
        setTotalCalories(result.totalCalories);
      } catch (error) {
        console.error("Erreur lors du chargement de la collation:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const EmptyList = () => (
    <View style={styles.emptyContainer}>
      ...
      <Text style={styles.emptySubtext}>
        Vous n'avez pas encore ajouté d'aliments pour la collation de cette journée.
      </Text>
      <TouchableOpacity 
        style={styles.addButton}
        onPress={() => router.push({
          pathname: "/(calorie)/Add",
          params: { mealType: "Collation", date: dateParam }
        })}
      >
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Détail de la collation</Text>
        <View style={{ width: 24 }} />
      </View>
      ...
      <View style={styles.summaryCard}>
        <View style={styles.summaryIconContainer}>
          <View style={[styles.mealIconCircle, {backgroundColor: COLORS.green}]}>
            <Ionicons name="ice-cream" size={22} color="#FFF" />
          </View>
        </View>
