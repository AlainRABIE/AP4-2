...
const DetailSoir = () => {
  ...
  useEffect(() => {
    const loadAliments = async () => {
      setLoading(true);
      try {
        const result = await fetchMealByDate(date, "Dîner");
        setAliments(result.meals);
        setTotalCalories(result.totalCalories);
      } catch (error) {
        console.error("Erreur lors du chargement du dîner:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const EmptyList = () => (
    <View style={styles.emptyContainer}>
      ...
      <Text style={styles.emptySubtext}>
        Vous n'avez pas encore ajouté d'aliments pour le dîner de cette journée.
      </Text>
      <TouchableOpacity 
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Détail du dîner</Text>
        <View style={{ width: 24 }} />
      </View>
