...
const DetailRepas = () => {
  ...
  useEffect(() => {
    const loadAliments = async () => {
      setLoading(true);
      try {
        const result = await fetchMealByDate(date, "Petit-déjeuner");
        setAliments(result.meals);
        setTotalCalories(result.totalCalories);
      } catch (error) {
        console.error("Erreur lors du chargement du petit-déjeuner:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const EmptyList = () => (
    <View style={styles.emptyContainer}>
      ...
      <Text style={styles.emptySubtext}>
        Vous n'avez pas encore ajouté d'aliments pour le petit-déjeuner de cette journée.
      </Text>
      <TouchableOpacity 
        style={styles.addButton}
        onPress={() => router.push({
          pathname: "/(calorie)/Add",
          params: { mealType: "Petit-déjeuner", date: dateParam }
        })}
      >
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Détail du petit-déjeuner</Text>
        <View style={{ width: 24 }} />
      </View>
